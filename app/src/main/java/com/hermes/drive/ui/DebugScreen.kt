...
@Composable
fun DebugScreen(log: String, onClear: () -> Unit, modifier: Modifier = Modifier) {
    Column(modifier = modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Button(onClick = onClear) { Text("Clear log") }
        Text("Hermes debug log (tag 'Hermes' in logcat). Long-press to copy.", style = androidx.compose.material3.MaterialTheme.typography.labelSmall)
